...
from brain.learning.self_evolving_planner import SelfEvolvingPlanner, ObservedAction
...
def demo_emotional_intelligence():
    ...
    print(f"  Recommendation: Give human space, minimize interaction")
...
def demo_predictive_maintenance():
    ...
    if failure_prob > 0.7:
        print(f"    [ALERT] High failure risk - schedule maintenance!")
...
def demo_cross_species_learning():
    ...
    print(f"  Principle: {principle}")
    print(f"  Robot now uses pheromone-inspired virtual markers!")
...
def demo_ethical_reasoning():
    ...
    from brain.learning.ethical_reasoning import EthicalReasoningEngine, EthicalOption
...
def demo_bio_neural():
    ...
    print(f"  Neuron types: sensory, motor, interneuron")
...
def demo_intention_prediction():
    ...
    from brain.learning.intention_prediction import IntentionPredictor, HumanContext
    from datetime import datetime
    ...
    print(f"  Proactive actions:")
    for action in prediction1.proactive_actions:
        print(f"    - {action}")
    print(f"  [ROBOT] Starting coffee maker before being asked!")
    ...
    print(f"  [ROBOT] Preparing entertainment options...")
